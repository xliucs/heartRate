...
public class HeartRateCameraView extends SurfaceView implements Callback, Camera.PreviewCallback {
    ...
    public void onPreviewFrame(byte[] data, Camera camera) {
        ...
        int[] redColor = new int[pixels.length];
        int sum = 0;
        int sum_third = 0;
        double mean = 0;
        ...
        for (int i = 0; i < pixels.length; i++) {
            redColor[i] = Color.red(pixels[i]);
            sum = sum + Color.red(pixels[i]);
        }

        //calculate mean value, if the length < 3, then calculate mean directly, if length is bigger than 3, then we use
        // for loop to calculate mean of middle 1/3 elements of the array.

        if(redColor.length<= 3){
            mean = sum / (redColor.length);
        }else{
            for (int i = (redColor.length)/3; i<(((redColor.length)/3)*2); i++){
                average.add(redColor[i]);
                sum_third = sum_third+redColor[i];
            }
            mean = sum_third/average.size();
        }

        for (PPGListener ppgLis : listeners){
            ppgLis.onSensorChanged(new PPGEvent(mean,System.currentTimeMillis()));
        }
    }
